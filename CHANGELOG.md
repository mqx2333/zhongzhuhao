...
## v0.5.2 —— 修复 GUI 窗口超出屏幕 / 内容被裁切

### 修复
- **窗口过高且定位过低，导致「开始处理」按钮与日志区被挤出屏幕**（高 DPI 下尤其明显）。
- 改为**可滚动内容区**（Canvas + 滚动条）：小屏 / 高 DPI 下也能访问全部控件。
- 按 Windows **工作区**（不含任务栏，`SPI_GETWORKAREA`）计算窗口尺寸与位置，确保完整可见。
- 图例并入「处理选项」卡片，整体更紧凑；默认窗口约 720 高。
- 后台线程异常捕获由 `Exception` 改为 `BaseException`：pandoc 失败时 core 抛的是
  `SystemExit`，旧版会**静默无提示**，现在会在日志中显示错误。
...
## v0.5.1 —— 修复默认黑色被误标为字体颜色

### 修复
- **不再把默认黑色 `#000000` 当作“字体颜色”标注。**
  Word 常给普通正文 run 显式写入 `w:color val="000000"`；旧版会把整篇正文包成
  `<span style="color:#000000">`，产生大量噪声（实测《齐桓晋文之事》学案中
  黑色 247 处、蓝色 116 处，正文几乎全被“染黑”）。
- 现在默认忽略 **黑 `#000000`** 与 **白 `#FFFFFF`**；实测同一文件修复后：
  黑色 span 0 个、蓝色 59 个、着重号 50 个，未命中 0。

### 新增
- 可配置忽略颜色：
  - CLI：`--ignore-colors "#000000,#FFFFFF"`（传空串 `""` 即不过滤，全部颜色都处理）
  - GUI：「处理选项」新增「忽略颜色」输入框
  - API：`Options.ignore_colors`、`DEFAULT_IGNORE_COLORS`、
    `extract_segments(..., ignore_colors=...)`
